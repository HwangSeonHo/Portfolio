			...
			<form name="f" method="post">
				...
				<table border="0" cellpadding="0" cellspacing="1" width="590"
					...
					<tr>
						...
						<td width=120 align=center bgcolor="E6ECDE">조회수</td>
					</tr>
...
<!-- ${listResultBean.isShowFirst()} 는 선택된 페이지가 1인지 아닌지 판단해줌. 만약에 선택된 페이지가 1이라면  ◀◀를 보여준다.-->
				
				<c:if test="${listResultBean.isShowPreviousGroup()}"> <!-- ${listResultBean.isShowPreviousGroup()}는 이전그룹의페이지를 보여줄것인지의 여부-->
				...
				<c:forEach var="pageNo" begin="${listResultBean.startPageNo }" end="${listResultBean.endPageNo}"  varStatus="status">
					<c:if test="${listResultBean.selectPageNo==status.index}"><!-- 선택된 페이지가 인덱스랑 같다면 폰트색은 red -->
							<font color='red'>
			...
			<table border="0" cellpadding="0" cellspacing="1" width="590">
				<tr>
					<td align="right"><input type="button" onclick="recipeCreateForm();" value="방명록 쓰기" /></td>
				</tr>
			</table></td>
			</tr>
</table>
