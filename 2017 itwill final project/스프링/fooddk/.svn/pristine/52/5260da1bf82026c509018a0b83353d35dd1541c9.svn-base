 ...
 /*     Item item = itemServiceImpl.selectByNo(Integer.parseInt(menu_no));
      ...
      for (int i=0; i<list.size();i++) {
    	 Map<String,Object> map = new HashMap<>();
    	  ...
    	  map.put("i_price", itemServiceImpl.selectByNo(list.get(i).getI_no()).getI_price());
    	  mapList.put(i, map);
    	  
    	  sum += itemServiceImpl.selectByNo(list.get(i).getI_no()).getI_price();
      }
      Map<String,Object> map = new HashMap<>();
      map.put("price", sum);
      mapList.put(9999, map);
