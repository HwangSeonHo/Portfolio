...
<jsp:include page="common_top.jsp"></jsp:include>
		...
		<!-- START CONTAINER
		==========================================-->
		<form name="r" method="post" action="recipe_search_list"  role="search" class="search-form">
		<div class="taxonomydownloadcategoryphp">
			<div class="container">
				...
				<!-- 	<div class="c12">
						...
						<div class="row" align="center">
							<div class="col-md-8 pagination">
									...
									<c:if test="${listResultBean.isShowPreviousGroup()}">
										<a href="recipe_list?c_no=${c_no}&&selectPage="
											${listResultBean.previousGroupStartPageNo}>prev</a>
									</c:if>


									<c:forEach begin="${listResultBean.startPageNo }"
										end="${listResultBean.endPageNo}" varStatus="status">
								...
								<a href="recipe_list?c_no=${c_no}&&selectPage=${status.index}">${status.index}</a>
										...
										<c:if test="${listResultBean.selectPageNo!=status.index}">
											<a
												href="recipe_list?c_no=${c_no}&&selectPage=${status.index}"
							...
							<%-- 	<a href="recipe_list?c_no=${c_no}&&selectPage=${status.index}"> ${status.index} </a> --%>
									...
									<c:if test="${listResultBean.isShowNextGroup()}">
										<a href="recipe_list?c_no=${c_no}&&selectPage="
											${listResultBean.nextGroupStartPageNo}>next</a>
									</c:if>

									<c:if test="${listResultBean.isShowLast()}">
										<a
											href="recipe_list?c_no=${c_no}&&selectPage=${listResultBean.totalPageCount}">last</a>
									</c:if>
							</div>
							...
							<div class="col-md-4" align="right">
								<div class="sortitemsarea fright">
									...
									<div id="secondary"
										...
										role="complementary" class="col-md-4">
										<aside class="widget widget_search" class="col-md-4">
											<!-- <form name="s" role="search" method="get" class="search-form"
												action="recipe_search_list">-->
												<input type="search" class="search-field" placeholder="Type and hit enter..." value="" name="r_title" title="" onsubmit="return true;"> 
												<input type="hidden" name="r_title"
													value="${searchList.r_title}">
											<!-- </form> -->
										</aside>
									</div>
								</div>
					...
					<script type="text/javascript">
					function search() {
						alert(document.getElementByName("r_title").value);
						document.r.action = "recipe_search_list";
						document.r.submit();
					}
					</script>
